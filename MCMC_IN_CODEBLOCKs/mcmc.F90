...
subroutine MCMCfit(calc_fit,calculate_chi)!, ind, min, max)
    ...
    INTEGER  ierr,rc, numprocs, myid

    logical :: flag                  ! mpi_var
...
! >>>>>>>>>>>>>>>>>>>>>>>>>>> initialize Monte Carlo module >>>>>>>>>>>>>>>>>>>>>>>
    ...
    if(myid == 0) then
        ...
        !flag = allocated(MCchain)

        if (allocated(MCchain)) deallocate(MCchain)
        !    do i=1,size(MCchain)
        !        deallocate(MCchain(i)%par)
        !        deallocate (MCchain(i)%chi)
        !        deallocate (MCchain(i)%auto)
        !        deallocate (MCchain(i)%ave)
        !        deallocate (MCchain(i)%disp)
        !        deallocate (MCchain(i)%med)
        !        deallocate (MCchain(i)%sca)
        !   end do
        !   deallocate
        !end if
        !if ((ind == 0) .or. (ind == mini)) then
        ...
        if (ind == 0) then
            ...
            nameMCchain = 'OUT/MCchain'//TRIM(ADJUSTL(charind))//'.dat'
        end if
...
! >>>>>> find chi_min and distribute it to all processes
        ...
        if(myid == 0) then
	        ...
	        write(FormatChar, "(A, I0, A)") '(1I4, 1I8,', 7+2*num_of_var, 'f19.9)'
	        write(18,FormatChar) iter, syn(1)%free_parameters, &
	        koef, chi_min, t*1.0d0/num_MC,  dlog10(x), 1.0d0*corr_lenght, MCchain(iter)%mom(1), &
	        MCchain(iter)%mom(2), (vari(k), k=1,2*num_of_var)
