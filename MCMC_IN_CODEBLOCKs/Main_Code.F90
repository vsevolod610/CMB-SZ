...
subroutine shift_MC(i_MC)
    ...
    case(2)              ! odnorodnoe distribution in [-1,+1]^num_of_var cube
        do k=1, num_of_var
            ...
            x(k) = -1.d0 + 2.d0*xx !commit: add d in 2.0
        end do
    end select
    ...
    if (calc_MCmin == 1) then
        ...
        if(write_MCmin == 1) then
            ...
            do k=1, num_MC
                ...
                write(FormatChar, "(A, I0, A)") '(', num_of_var+1, 'f20.10)'
                write (867,FormatChar) MCres(num_of_var+1), (MCres(i), i=1,num_of_var)
            enddo
            MC = MCsave
            close(867)
        end if
...
!>>>>>>>>>>>>     stats of chain
    if(stats_chain == 1) then
        ...
        do k=1, num_MC
            call write_char_MC(k, buffer)
            MCchain(iter)%par(k,:) = MCres(1:num_of_var)
    ...
    if ((calc_Autocorr == 1) .and. (iter > 100)) then
        ...
        do t=0, min(iter/4, 400)
            do l=1, num_of_var
                do i=iter/2, iter-t
                !do k=1, num_MC
                    Autocorr(t+1,l) = Autocorr(t+1,l) + (MCchain(i+t)%par(k,l) - &
                    MCchain(iter)%med(k,l))*(MCchain(i)%par(k,l) - MCchain(iter)%med(k,l))
                !enddo
                enddo
            Autocorr(t+1,l) = Autocorr(t+1,l)/(MCchain(iter)%sca(k,l) - MCchain(iter)%med(k,l)**2)/((iter+1)/2)
            enddo
            write(FormatChar, "(A, I0, A)") '(1I4,', num_of_var, 'f20.9)'
            write(24, FormatChar) t, (Autocorr(t+1,l), l=1,num_of_var)
        enddo
        close(24)
    endif
...
! >>>>>>>>>>>  Gelman-Rubin test
    if (calc_GRtest == 1) then
        if (iter > 3) then
            call GRtest
            write(FormatChar, "(A, I0, A)") '(1I4,', num_of_var, 'f20.9)'
            write(20,FormatChar) iter, (GRvalue(1,k), k=1,num_of_var)
            write(FormatChar, "(A, I0, A)") '(1I4,', 2*num_of_var,'f20.9)'
            write(21,FormatChar) iter, (GRvalue(2,k), GRvalue(3,k), k=1,num_of_var)
        endif
    endif
...
!   >>>>>>>>>>>>>>> write MC
    if (write_data == 1) then
        ...
        do k=1, num_MC
            write(FormatChar, "(A, I0, A)") '(', num_of_var+1, 'f20.10)'
            if (write_data == 1) write (667,FormatChar) MCchain(iter)%chi(k), (MCchain(iter)%par(k,i), i=1,num_of_var) !, MCchain(iter)%auto(k)
        ...
        if (iter/20 == iter*1.0d0/20) then
            ...
            if (ind == 0) then
                ...
                nameChainfile = 'OUT/chainconsum'//TRIM(ADJUSTL(charind))//'.dat'
            end if
...
!   >>>>>>>>>>>> write some additional MC files
    if (show_addMC == 1) then
            ...
            do i=5, iter
                if (MCchain(i)%auto(k) == 1) then
                    if (MCchain(i)%chi(k) < chi_emin + koef) then
        			    write(FormatChar, "(A, I0, A)") '(2i10,', num_of_var+1, 'f20.10)'
        			    write (7,FormatChar) i, k, MCchain(i)%chi(k), (MCchain(i)%par(k,l), l=1,num_of_var)
                    endif
                endif
            enddo
        enddo
        ...
        if(iter == 1) then
            ...
            do k = 1, num_MC
                write(FormatChar, "(A, I0, A)") '(2i10,', num_of_var+1, 'f20.10)'
	            write (7,FormatChar) iter, k, MCchain(iter)%chi(k), (MCchain(iter)%par(k,l), l=1,num_of_var)
            end do
            close(7)
        end if
    end if
...
! >>>>>>>>>>>>>>  calculate distribution of the parameters: average and dispersion
    if (calc_stats == 1) then
        ...
        write(FormatChar, "(A, I0, A)") '(1I4,', 3*num_of_var, 'f20.9)'
        write(19, FormatChar) iter, (MCchain(iter)%ave(i), i=1,num_of_var), &
        (MCchain(iter)%disp(i), i=1,num_of_var), (MCchain(iter)%par(1,l), l=1,num_of_var)
...
subroutine shiftAffine_MC(i,z,s)
    ...
    integer i,s,j,t,k
    double precision x, z, i_min,i_max
    double precision, external :: drandm
    if (z == 0) then
        if (s == 0) then
            !x = drandm(0)
    ...
    if (s == 0) call RANDOM_NUMBER(x)!call RANDOM_NUMBER(x)!x = drandm(0)
    if (s == 1) x = random(4,i)
    if (s == 2) x = random(5,i)
...
subroutine modifyMC_Affine(corr_limit, calc_fit, calculate_chi)
    ...
    double precision tune_old, chix, x,  z, rand
    double precision, external :: drandm
