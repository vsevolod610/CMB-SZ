 ...
 !   k = k -1
    ...
    do j = 2, num_J+1
        do i  = 1, j-1
            ...
            if(k_ind == 1) Matrix_coll(i,j,2) = qCOH2o(m,k) + &
            (qCOH2o(m,k+1)-qCOH2o(m,k))*(T_kin-qCOH2o(1,k))/(qCOH2o(1,k+1)-qCOH2o(1,k))   ! linear appr
            if(k_ind == 1) Matrix_coll(i,j,3) = qCOH2p(m,k)+ &
            (qCOH2p(m,k+1)-qCOH2p(m,k))*(T_kin-qCOH2p(1,k))/(qCOH2p(1,k+1)-qCOH2p(1,k))
        end do
    end do
...
!    k = k -1
    ...
    do m = 2,106
        ...
        if((i.le.num_J+1).and.(j.le.num_J+1)) then
            ...
            if(k_ind == 1) Matrix_coll(j,i,4) = qCOHe(m,k+2) + &
            (qCOHe(m,k+3)-qCOHe(m,k+2))*(T_kin-qCOHe(1,k))/(qCOHe(1,k+1)-qCOHe(1,k))   ! linear appr
        end if
    end do
    ! for i<j
    ...
    do i =2, num_J+1
           ...
           Matrix_cmb(i,i-1) = AJ_CO(i)*dexp(-(EJ_CO(i)-EJ_CO(i-1))/T_cmb)/&
           (1- dexp(-(EJ_CO(i)-EJ_CO(i-1))/T_cmb))*(2.0*(i-1)+1.0)/(2.0*(i-2)+1.0)
   ...
   !#########################################  the finish of matrix determination
    ...
    do i =1, num_J+1
        do j = 1, num_J+1
            Matrix(i,j) = Matrix_CMB(i,j)* beta_factor + n1*Matrix_coll(i,j,1)+&
            n_ortho*Matrix_coll(i,j,2)+n_para*Matrix_coll(i,j,3)+n3*Matrix_coll(i,j,4)
