...
subroutine read_abgrall1982
    ...
    do while (.not. eof(12))
        read(12,'(1A200)') buf
        read(buf(1:12),'(1i7,1i5)') i, j
        if (i > j ) then 
            read(buf(15:56),'(3e14.3)') a_coll(k,i,j), b_coll(k,i,j), c_coll(k,i,j)  
        end if
    end do
    close(12)
    ...
    do while (.not. eof(12))
        read(12,'(1A200)') buf
        read(buf(1:6),'(1i3,1i3)') i, j
        if (i > j ) then 
            read(buf(7:39),'(3e11.3)') a_coll(k,i,j), b_coll(k,i,j), c_coll(k,i,j)  
        end if
    end do
    close(12)
    end
...
!    pause
    ...
    do i =1, Jmax
        do j = 1, Jmax
            mval = Matrix_uv(i,j)*chi_UV*f_shi + n(1)*Matrix_coll(i,j,1)+n(2)*Matrix_coll(i,j,2) + n(3)*Matrix_coll(i,j,3)+n(4)*Matrix_coll(i,j,4) + n(5)*Matrix_coll(i,j,5) + Matrix_sp_deexc(i,j)
