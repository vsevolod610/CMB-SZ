...
def pic_fit(sampler, model, data, prior_data=None, path=False):
    ...
    if 1:
        ...
        y_list = np.array([model(*w, *const, x) for w in params_3sigma])
        y3_min = np.array(list(map(min, y_list.T)))
        y3_max = np.array(list(map(max, y_list.T)))

        ## FIXME: if model return array ???
        #print(model(*params_3sigma[0], *const, x[0]))
